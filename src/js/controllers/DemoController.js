app.controller("DemoController", function ($scope, $http, $log, AppConfig) {
  ...
  $scope.emailData = ["aaa.kumar@gmail.com", "bbb.kumar@gmail.com", "ccc.kumar@gmail.com"];
  ...
  $scope.init = function () {
    $http.get("data/json/linkData.json").then(function (response) {
      $scope.linkData = response.data;
      //$log.log("Succsss : status " + response.status + " data " + angular.toJson(response.data));
    }, onError);

    $http.get("data/json/dbBakupLinkData.json").then(function (response) {
    ...
    $http.get("data/json/importResponse.json").then(function (response) {
