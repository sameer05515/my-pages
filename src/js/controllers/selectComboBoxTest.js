app.controller("selectComboBoxTest", function ($scope, $http, $log, AppConfig, InterviewManagementServices) {
  ...
  $scope.refreshAllCategories = function () {
    $http.get(AppConfig.interviewMgmtServices + "/categories/optimized/opti").then(function (response) {
      $scope.categoryData = [];
      angular.forEach(
        response.data,
        function (val, key) {
  ...
  $scope.showCategoryQuestions = function (givenCategoryObject) {
    ...
    $http.get(AppConfig.interviewMgmtServices + "/categories" + "/" + givenCategoryObject.catID).then(function (response) {
